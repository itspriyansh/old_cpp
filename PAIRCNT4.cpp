...
using namespace std;
int timein[100000], height[100000], counts=0, cr[100000], sparse[19][200000], path[200000];
...
int main() {
	...
	while(t--) {
		...
		while(q--) {
			...
			ch.pb(height[cr[0]]);
			for(int i=1;i<k;i++) {
				// int x = query(timein[cr[i-1]], timein[cr[i]]);
				ch.pb(query(timein[cr[i-1]], timein[cr[i]]));
				ch.pb(height[cr[i]]);
			}
			for(int i=0;i+2<ch.size();i+=2) {
				int mn = ch[i+1];
				for(int j=i+2;j<ch.size();j+=2) {
					mn = min(mn, ch[j-1]);
					if(ch[i]+ch[j] == d+2*mn) {
						ans++;
					}
